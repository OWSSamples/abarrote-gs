...
async function _importProductsFromCSV(formData: FormData, overwrite: boolean, _publish: boolean) {
  try {
    ...
    return {
      success: false,
      message: error instanceof Error ? error.message : 'Error desconocido al importar el archivo CSV',
    };
  }
}
...
async function _importCustomersFromCSV(formData: FormData) {
  try {
    ...
    if (!hasId && !hasEmail && !hasPhone) {
      throw new Error(
        'Falta un encabezado obligatorio. Debe estar presente al menos uno de los siguientes encabezados: Customer ID, Email, Phone.',
      );
    }
    ...
    return {
      success: false,
      message: error instanceof Error ? error.message : 'Error desconocido al importar clientes',
    };
  }
}
