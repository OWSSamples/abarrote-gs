...
export const saveStoreConfigSchema = z
  .object({
    ...
    cfdiPacProvider: z.string().max(50).optional(),
    cfdiPacEnvironment: z.enum(['sandbox', 'production']).optional(),
    cfdiPacAuthType: z.enum(['basic', 'bearer', 'api-key']).optional(),
    cfdiPacApiUrl: z.string().max(500).optional(),
    cfdiPacApiKey: z.string().max(500).optional(),
    cfdiPacApiSecret: z.string().max(500).optional(),
    cfdiPacCancelPath: z.string().max(120).optional(),
    ...
    openCashDrawerOnCashSale: z.boolean().optional(),
    ...
    salesScheduleEnabled: z.boolean().optional(),
    salesOpenTime: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/).optional(),
    closeSystemTime: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/).optional(),
    autoCorteEnabled: z.boolean().optional(),
    autoCorteTime: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/).optional(),
    businessTimezone: z
      .enum([
        'America/Mexico_City',
        'America/Cancun',
      ...
      .optional(),
    defaultStartingFund: z.number().finite().min(0).max(1_000_000).optional(),
    // Customer Display
