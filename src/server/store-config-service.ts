...
import { ValidationError, withLogging } from '@/lib/errors';
import { db } from '@/db';
...
import { STORE_SECRET_FIELDS, STORE_SECRET_MASK } from '@/lib/store-config-public';
...
const SYSTEM_MANAGED_CONFIG_FIELDS = new Set(['createdAt', 'updatedAt', 'deletedAt']);
...
function mapStoreConfigRow(row: any): StoreConfig {
  return {
    ...
    phone: row.phone,
    country: row.country ?? DEFAULT_STORE_CONFIG.country,
    businessType: row.businessType ?? DEFAULT_STORE_CONFIG.businessType,
    businessTypeOther: row.businessTypeOther ?? undefined,
    contactEmail: row.contactEmail ?? undefined,
    estimatedUsers: row.estimatedUsers ?? DEFAULT_STORE_CONFIG.estimatedUsers,
    rfc: row.rfc,
    ...
    telegramWebhookSecret: row.telegramWebhookSecret ?? undefined,
...
async function _fetchStoreConfig(): Promise<StoreConfig> {
  ...
  const config = await fetchStoreConfigById(storeId, true);
  if (!config) throw new Error('No fue posible cargar la configuración de la tienda.');
  return config;
}

async function _fetchStoreConfigForStore(storeId: string): Promise<StoreConfig | null> {
  return fetchStoreConfigById(storeId, false);
}
...
async function _saveStoreConfig(data: Partial<StoreConfig>): Promise<StoreConfig> {
  ...
  const systemManagedFields = Object.keys(data).filter((field) => SYSTEM_MANAGED_CONFIG_FIELDS.has(field));
  if (systemManagedFields.length > 0) {
    throw new ValidationError('La configuración contiene campos administrados por el sistema.', {
  ...
  for (const field of STORE_SECRET_FIELDS) {
    ...
    if (value === undefined || value === STORE_SECRET_MASK || (typeof value === 'string' && value.trim() === '')) {
  ...
  const persist = async (dbValues: Record<string, unknown>) => {
    const result = await db.update(storeConfig).set(dbValues).where(eq(storeConfig.id, storeId));
    if (!result.rowCount || result.rowCount === 0) {
      await db.insert(storeConfig).values({ id: storeId, ...dbValues });
    }
  };
  ...
  for (const key of changedKeys) {
    const isSecretField = (STORE_SECRET_FIELDS as readonly string[]).includes(key);
    emitDomainEvent({
      type: 'config.changed',
      payload: {
        field: key,
        before: undefined,
        after: isSecretField ? '[REDACTED]' : (fields as Record<string, unknown>)[key],
      },
      metadata: { userId: user.uid, userEmail: user.email ?? '', storeId },
    });
  }

  const saved = await fetchStoreConfigById(storeId, false);
  if (!saved) throw new Error('No fue posible recuperar la configuración guardada.');
  return saved;
}

// ==================== SERVER-ONLY API ====================

export const getStoreConfig = withLogging('storeConfig.getStoreConfig', _fetchStoreConfig);
export const getStoreConfigForStore = withLogging(
  'storeConfig.getStoreConfigForStore',
  _fetchStoreConfigForStore,
);
export const saveStoreConfigForOwner = withLogging('storeConfig.saveStoreConfig', _saveStoreConfig);
